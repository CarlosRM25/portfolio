...
Ranked. The first item is mechanical and should happen before anything else.

1. **Land `card-visuals-and-cleanup`.** PR #6 was merged when it held only its
   first commit, so three later commits never reached `main` and production is
   missing all of them: the agent card's visual, the diagram's OCR label fix,
   and the stale-copy cleanup. They were cherry-picked onto a fresh branch off
   the current `main` (so they do not revert the custom domain, which landed in
   between) and opened as a new PR. Merge that, confirm the deploy, then delete
   `hero-fixes-and-timecard-page`, whose PR is closed and which will otherwise
   keep looking like it has unmerged work.

2. **Put `steps` in the agent's `examples.json`.** The best single upgrade left,
...
3. **A real `og:image`.** `public/og.png` is still the generic card from
...
4. **Run the five-second test.** The 2026-09-11 notes asked for it before
...
5. **Check `PUBLIC_AGENT_API_URL` on Vercel Preview.** If it is set for
